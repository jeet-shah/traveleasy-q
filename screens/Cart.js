...
export default class Cart extends React.Component{
    ...
    paymentstatus = async() => {
        db.collection('Cart').doc(this.state.userID).collection('PaymentStatus').where("TotalPrice","==",0).get()
        .then(snapshot => {
        ...
        db.collection('Cart').doc(this.state.userID).collection('PaymentStatus').doc(this.state.docid8).update({
                ...
                <View style={styles.container}>
                    ...
                    <TouchableOpacity style={styles.button} onPress={()=>{
                        if(parseInt(this.state.PantPrice + this.state.ShirtPrice + this.state.WatchPrice + this.state.TiePrice + this.state.SportPrice + this.state.FormalPrice) === 0){
                            ...
                            db.collection('Status').where("TotalPrice","==",0).get()
                            .then(snapshot => {
                                snapshot.docs.forEach(doc => {
                                    let docid = doc.id
                                    this.setState({
                                    ...
                                    console.log(this.state.docid7)
                                })
                            })
                            db.collection('Status').doc(this.state.docid7).update({
